...
flatdata = np.zeros((36, 10), dtype=str)
average = np.zeros((10, max), dtype=int)

for n1 in range(10):
    index = 0
    images = glob.glob('C:/Users/bruno/Documents/Python/Options Research/numbers/15x20/White/%s/*.png'%n1)
    for image in images:
        with open(image, 'rb') as file:
            ...
            flatdata[index, n1] = binary
        count[n1] += 1
        index += 1

np.savetxt("15x20White.csv", flatdata, fmt="%s", delimiter = ",")
